...
import {
  COLOR_PROFILE_FOLDER,
  PATTERN_FILL_IMAGES_FOLDER,
  SETTINGS_PATH,
} from "./constants";
...
export async function createPatternFillImages(): Promise<void> {
  ...
  const settings = JSON.parse(await fs.readFile(resolveProjectPath(SETTINGS_PATH), "utf8")) as Record<string, number>;
  const fillOpacity = Math.floor(settings.FILL_OPACITY * 255).toString(16).padStart(2, "0");
  ...
  for (const file of files) {
    const data = JSON.parse(await fs.readFile(path.join(colorProfileFolder, file), "utf8")) as ColorProfile;
    const profileName = file == "default.json" ? "none" : path.basename(file, ".json");

    for (const [size, patternSize] of patternSizes) {
      for (const colorKey of colorKeys) {
        writeTasks.push(writePatternImage(data, fillOpacity, profileName, patternSize, colorKey, size));
      }
    }
  }
